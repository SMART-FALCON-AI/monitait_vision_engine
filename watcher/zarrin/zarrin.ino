...
const int MAX_ENCODER_EVENTS = 20;
...
volatile long raw_encoder_value = 0;
long delayed_encoder_value = 0;
unsigned long encoder_update_time = 0;
const unsigned long encoder_delay_ms = 12500;
...
void count_up_a(){
  ...
  if (j > 180){
    ...
    int inc = (digitalRead(input_b) == HIGH) ? -1 : 1;

    encoder_events[encoder_head] = {millis() + encoder_delay_ms, inc};
...
void count_up_b(){
  ...
  if (j > 110){
    ...
    int inc = (digitalRead(input_a) == HIGH) ? 1 : -1;
...
void process_encoder_events() {
  ...
  while (encoder_tail != encoder_head && encoder_events[encoder_tail].scheduled_time <= now) {
