...
// 3.21.18 — Live Channels strip on the Dashboard tab. Polls /api/live_channels
// every 1.5s and renders a chip per Show=ON class with the latest value, age
// dot, and severity background. This lets math channels (Store=OFF, no DB) show
// up live on the dashboard without any persistence.
let _liveChannelsTimer = null;
async function refreshLiveChannels() {
    const card = document.getElementById('live-channels-card');
    const strip = document.getElementById('live-channels-strip');
    const countEl = document.getElementById('live-channels-count');
    if (!card || !strip) return;
    let data;
    try {
        const r = await fetch('/api/live_channels?max_age_sec=30');
        if (!r.ok) return;
        data = await r.json();
    } catch (e) { return; }

    const channels = data.channels || {};
    const names = Object.keys(channels).sort((a, b) => (channels[b].ts || 0) - (channels[a].ts || 0));
    if (!names.length) {
    ...
    // Build the chip HTML for each channel. Math channels get amber accent,
    // YOLO classes get blue. Age dot: green < 3s, amber < 10s, red older.
    const html = names.map(name => {
        const c = channels[name] || {};
        const v = c.value;
        const isMath = !!c.is_math;
        const accent = isMath ? '#fbbf24' : '#60a5fa';
        let val = '—';
        if (v != null) {
            val = isMath
        ...
        const age = c.age_sec ?? 999;
        const ageColor = age < 3 ? '#22c55e' : (age < 10 ? '#f59e0b' : '#ef4444');
        const sev = c.severity || 0;
        const sevSuffix = sev > 0 ? ` · sev ${sev}` : '';
        const cam = c.camera ? ` · ${c.camera}` : '';
        return `
            <div title="${name}${cam}${sevSuffix} · ${age}s ago" style="display:flex; align-items:center; gap:6px; padding:5px 10px; background:rgba(15,23,42,0.6); border:1px solid ${accent}55; border-radius:14px; font-size:12px;">
                <span style="width:7px; height:7px; border-radius:50%; background:${ageColor}; flex:none;"></span>
                <span style="color:${accent}; font-weight:600;">${name}</span>
                <span style="color:#e2e8f0; font-weight:600; font-variant-numeric:tabular-nums;">${val}</span>
            </div>`;
    }).join('');
    strip.innerHTML = html;
}
window.refreshLiveChannels = refreshLiveChannels;
function startLiveChannelsLoop() {
    if (_liveChannelsTimer) clearInterval(_liveChannelsTimer);
    refreshLiveChannels();
    _liveChannelsTimer = setInterval(refreshLiveChannels, 1500);
}
function stopLiveChannelsLoop() {
    if (_liveChannelsTimer) { clearInterval(_liveChannelsTimer); _liveChannelsTimer = null; }
}
window.startLiveChannelsLoop = startLiveChannelsLoop;
window.stopLiveChannelsLoop = stopLiveChannelsLoop;
document.addEventListener('DOMContentLoaded', () => setTimeout(startLiveChannelsLoop, 1500));
